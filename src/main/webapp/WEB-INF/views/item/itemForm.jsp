...
<head>
    <link href="${pageContext.request.contextPath}/resources/css/item.css?v=<%System.currentTimeMillis(); %>"
        rel="stylesheet" />
</head>
...
<div class="container">
    ...
    <main class="item-Form-main">
        <div>
            ...
            <button class="ListInsert-btn" onclick="insertItem('${ItemType}')">${ItemType} 등록</button>
            <c:if test="${ItemType eq 'package'}">
                <button class="List-btn" onclick="location.href='${pageContext.request.contextPath}/item/paclist.bo'">패키지
                    리스트</button>
            </c:if>
            <c:if test="${ItemType eq 'item'}">
                <button class="List-btn"
                    onclick="location.href='${pageContext.request.contextPath}/item/itemlist.bo'">상품 리스트</button>
            </c:if>
        </div>
    </main>
</div>
...
<script>
    const insertItem = (itemType) => {
        const form = document.forms.frm;
        form.method = 'post';
        form.action = '${pageContext.request.contextPath}/item/insertItem.bo?ItemType=' + itemType;
        form.submit();
    }
</script>
