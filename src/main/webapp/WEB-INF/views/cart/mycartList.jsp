					...
					<c:otherwise>
						<c:forEach items="${cartItemList}" var="Item">
							<tr>
								...
								<td>
									<a class="cart-go-" href="${pageContext.request.contextPath}/item/item.bo?itemNo=${Item.itemNo}" >
						     			<img src="${pageContext.request.contextPath}/resources/images/itemlogo/${Item.itemLogo}" alt="logo"> 
									</a>
								</td>
								<td>
									<a href="${pageContext.request.contextPath}/item/item.bo?itemNo=${Item.itemNo}" >
							...
							<c:otherwise>
								<c:forEach items="${cartPacList}" var="Pac">
									<tr>
										...
										<td>
											<a href="${pageContext.request.contextPath}/item/ipacItem.bo?itemNo=${Pac.itemNo}" >
...
window.addEventListener('resize', function() {
	const cartBarBox = document.querySelector('.cart-bar-box');
	const cartContainer = document.querySelector('.cart-container');
	const containerTop = cartContainer.offsetTop;
	const scrollTop = window.pageYOffset || document.documentElement.scrollTop;

	if (scrollTop >= containerTop) {
		cartBarBox.style.position = 'fixed';
		cartBarBox.style.top = '10px'; // 원하는 위치로 조정
	} else {
		cartBarBox.style.position = 'static';
	}
});
</script>
